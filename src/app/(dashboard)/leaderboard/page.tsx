...
const MILESTONES = [
  ...
  { label: "Silver", xp: "1,500 XP", color: "text-gray-600 dark:text-gray-300" },
    ...
    <div className="flex flex-col gap-4">
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {XP_RULES.map((rule) => (
            <div key={rule.label} className="rounded-md bg-white/60 p-2.5 dark:bg-white/5">
              <p className="text-xs font-medium text-gray-500 dark:text-gray-400">{rule.label}</p>
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-900 dark:text-white">Want to climb the ranks?</p>
            <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
          ...
          <Link
            href="/challenges"
            className="shrink-0 rounded-full bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500 cursor-pointer"
          >
            Start Challenges
          </Link>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {MILESTONES.map((m) => (
          <Card key={m.label} className="p-3 text-center">
            ...
            <p className="text-xs text-gray-500">{m.xp}</p>
          </Card>
        ))}
      </div>
    </div>
