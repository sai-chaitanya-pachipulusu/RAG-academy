...
export default function StatsPage() {
  ...
  if (authLoading || loading) return <div className="flex items-center justify-center py-20"><p className="text-sm text-gray-500">Loading stats...</p></div>;
  if (!user) return <div className="flex items-center justify-center py-20"><p className="text-sm text-gray-500">Sign in to view stats</p></div>;

  const getColor = (count: number) => count === 0 ? 'bg-gray-100' : count === 1 ? 'bg-green-200' : count <= 3 ? 'bg-green-400' : count <= 5 ? 'bg-green-500' : 'bg-green-600';

  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-xl font-semibold tracking-tight">Your Performance</h1>
        <p className="mt-0.5 text-sm text-gray-500">Track your progress and submissions.</p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
            ...
            return (
              <div key={key} className="mb-2">
                <div className="flex justify-between text-xs mb-0.5">
                  ...
                  <span className="text-gray-500">{difficultyBreakdown[key]}/{totalChallenges[key]}</span>
                </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/challenges" className="text-xs text-blue-600 hover:underline cursor-pointer">View all</Link>
          </div>
          <div className="space-y-1.5">
            {submissions.slice(0, 8).map((s) => {
              ...
              return (
                <Link key={s.id} href={`/challenges/${s.challengeSlug}`} className="flex items-center justify-between p-2 rounded-md bg-gray-50 hover:bg-gray-100 transition-all duration-200-all duration-200 cursor-pointer">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400">{ago} ago</p>
                    </div>
                  </div>
            ...
            {submissions.length === 0 && <p className="text-xs text-gray-500 text-center py-4">No submissions yet</p>}
          </div>
        </div>
      </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              { label: 'Other', count: submissions.filter(s => !['accepted', 'wrong_answer', 'runtime_error'].includes(s.status)).length, color: 'text-gray-600' },
            ].map((b) => (
              <div key={b.label} className="text-center p-2 rounded-md bg-gray-50">
