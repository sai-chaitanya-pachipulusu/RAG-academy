    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="h-10 w-full rounded-xl border border-zinc-100 bg-zinc-50/50 pl-10 pr-16 text-sm outline-none transition-all focus:border-zinc-300 focus:bg-white"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md px-1.5 py-0.5 text-xs text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600"
            >
              Clear
            </button>
          )}
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="h-9 rounded-lg border border-zinc-100 bg-white px-3 text-xs font-medium text-zinc-600 outline-none hover:border-zinc-200 cursor-pointer"
          >
      ...
      <div className="flex items-center justify-between text-xs">
        <p className="text-zinc-500">
          <span className="font-bold text-zinc-900">{filteredChallenges.length}</span> challenges
        </p>
        <p className="text-zinc-500">
          ...
          return (
            <Reveal key={challenge.slug} delayMs={Math.min(i * 30, 300)}>
              <Link
                ...
                className={`group flex flex-col h-full rounded-xl border bg-white p-4 transition-all hover:shadow-lg hover:shadow-zinc-100/50 ${
                  isLocked 
                    ? "border-zinc-100 opacity-75 hover:opacity-100 hover:border-zinc-200" 
              ...
              >
                ...
                {/* Content */}
                <h3 className="text-sm font-bold text-zinc-900 group-hover:text-zinc-600 transition-colors line-clamp-1">
                  {challenge.title}
                </h3>
                <p className="mt-1 text-xs leading-relaxed text-zinc-500 line-clamp-2 flex-1">
      ...
      {filteredChallenges.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-bold text-zinc-900">No challenges found</h3>
