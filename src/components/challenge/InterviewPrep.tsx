...
export function InterviewPrep() {
  ...
  if (status === "ready" && session) {
    return (
      <Reveal>
        <div className="mx-auto max-w-3xl rounded-2xl border border-zinc-900 bg-zinc-900 p-6 text-white shadow-xl">
          <div className="mb-6 flex items-center gap-4">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-zinc-900 text-lg font-bold">
              ⚡
            </div>
            <div>
          ...
          <div className="mb-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {session.challenges.map((c, i) => (
              <div key={c.slug} className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 p-2">
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-white text-[10px] font-bold text-zinc-900">
                  {i + 1}
                </span>
          ...
          <div className="flex gap-2">
            <Link
              ...
              className="flex-1 rounded-lg bg-white py-3 text-center text-sm font-bold text-zinc-900 transition-all hover:bg-zinc-100"
            >
              Start
            </Link>
            <button
              onClick={() => { setSession(null); setStatus("idle"); }}
              className="rounded-lg border border-white/20 px-4 py-3 text-sm font-semibold text-zinc-400 transition-all hover:bg-white/5"
            >
              Cancel
            </button>
          </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Quick Start</h3>
            ...
            return (
              <Reveal key={preset.id}>
                ...
                >
                  ...
                  <h4 className="text-sm font-bold text-zinc-900">{preset.name}</h4>
                  ...
                  <div className="mt-3 flex items-center justify-between text-[9px] font-bold uppercase tracking-widest">
                    <span className="text-zinc-400">{preset.count} challenges</span>
                    <span className="text-zinc-900">~{preset.time}m</span>
                  </div>
                </button>
              </Reveal>
            );
      ...
      <Reveal>
        <div className="rounded-2xl border border-zinc-100 bg-zinc-50/50 p-5">
          <div className="grid gap-6 lg:grid-cols-2">
            {/* Difficulty */}
            <div>
              <h4 className="mb-3 text-[10px] font-bold uppercase tracking-widest text-zinc-400">Difficulty</h4>
                  ...
                  <button
                    ...
                    className={`rounded-lg px-4 py-2 text-xs font-semibold capitalize transition-all ${
                      difficulty === d
                        ...
                        : "bg-white border border-zinc-100 text-zinc-600 hover:border-zinc-200"
                    }`}
                  >
                    {d}
                  </button>
            ...
            {/* Count */}
            <div>
              <h4 className="mb-3 text-[10px] font-bold uppercase tracking-widest text-zinc-400">Count</h4>
          ...
          {/* Summary & Start */}
          <div className="mt-6 flex flex-col items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-white p-4 sm:flex-row">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Session</p>
            ...
            <button
              ...
              className="w-full rounded-lg bg-zinc-900 px-6 py-2.5 text-sm font-bold text-white transition-all hover:bg-zinc-800 sm:w-auto"
            >
              Generate Session
            </button>
          </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {TIPS.map((tip, i) => (
          <Reveal key={tip.title} delayMs={i * 30}>
            <div className="rounded-xl border border-zinc-100 bg-white p-4 space-y-1">
              <div className="h-px w-4 bg-zinc-200" />
              <h5 className="text-xs font-bold text-zinc-900">{tip.title}</h5>
