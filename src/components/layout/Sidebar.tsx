  ...
  return (
    <aside className="hidden w-56 shrink-0 lg:block">
      <nav className="sticky top-20 space-y-0.5">
        {NAV.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`group flex items-center gap-3 rounded-lg px-3 py-2.5 transition-all duration-200-all duration-200 ${
                active
        ...
        <Link
          href="/settings"
          className="group flex items-center gap-3 rounded-lg px-3 py-2 text-[15px] text-[var(--gray-400)] transition-all duration-200-all duration-200 hover:bg-[var(--gray-50)] hover:text-[var(--foreground)] cursor-pointer"
        >
