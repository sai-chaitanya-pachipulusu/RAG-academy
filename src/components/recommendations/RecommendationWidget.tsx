      ...
      <Card className="group relative overflow-hidden p-4 transition-all hover:shadow-md">
        ...
        <div className="pr-10">
          ...
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-500">{challenge.group}</span>
          </div>

          {/* Title & Description */}
          <h4 className="font-semibold group-hover:text-blue-600 dark:group-hover:text-blue-400">
            {challenge.title}
          </h4>
          ...
          {reasons.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {reasons.slice(0, 2).map((reason, index) => (
                <span
                  key={index}
                  className="inline-flex items-center gap-1 rounded-full bg-zinc-100 px-2 py-1 text-xs text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400"
                >
...
function ReviewCard({ recommendation }: { recommendation: ChallengeRecommendation }) {
  ...
  return (
    <Link href={`/challenges/${challenge.slug}`}>
      <Card className="group flex items-center gap-4 p-3 transition-all hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-emerald-100 dark:bg-emerald-900/30">
          <RotateCcw className="h-5 w-5 text-emerald-600" />
        </div>
        <div className="flex-1 min-w-0">
          <h4 className="truncate font-medium group-hover:text-emerald-600 dark:group-hover:text-emerald-400">
            {challenge.title}
          </h4>
...
function CompactRecommendationItem({ recommendation }: { recommendation: ChallengeRecommendation }) {
  ...
  return (
    <Link
      href={`/challenges/${challenge.slug}`}
      className="flex items-center gap-3 rounded-lg p-2 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-900/50"
    >
