    ...
    <Card className="p-6">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <label className="text-sm text-zinc-600 dark:text-zinc-400">k parameter:</label>
          <select
            value={k}
            onChange={(e) => {
      ...
      {showFormula && (
        <div className="mb-4 rounded-lg bg-zinc-100 p-4 dark:bg-zinc-800">
          <p className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">RRF Formula:</p>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {/* Dense results */}
        <div className={`transition-opacity ${animationStep !== "idle" ? "opacity-100" : "opacity-60"}`}>
          ...
          <div className="space-y-2 rounded-lg border border-blue-200 bg-blue-50/50 p-3 dark:border-blue-800 dark:bg-blue-950/30">
            {DENSE_RESULTS.map((result) => (
              <div
                key={result.id}
                className={`flex items-center justify-between rounded-lg border bg-white p-2 text-sm transition-all dark:bg-zinc-900 ${
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-zinc-700 dark:text-zinc-300">{result.title}</span>
                </div>
                {animationStep !== "idle" && (
        ...
        {/* Fused results */}
        <div className={`transition-opacity ${animationStep === "done" ? "opacity-100" : "opacity-40"}`}>
          ...
          <div className="space-y-2 rounded-lg border border-purple-200 bg-purple-50/50 p-3 dark:border-purple-800 dark:bg-purple-950/30">
            {fusedResults.slice(0, 6).map((result, idx) => (
              <div
                key={result.id}
                className={`flex items-center justify-between rounded-lg border bg-white p-2 text-sm dark:bg-zinc-900 ${
                  animationStep === "done"
