    ...
    <Card className="p-6">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="w-24"
          />
          <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{threshold.toFixed(1)}</span>
        </div>
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            className="rounded"
          />
          <span className="text-zinc-600 dark:text-zinc-400">Highlight relevant only</span>
        </label>
      </div>
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        <div className="rounded-lg border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <p className="text-xl font-bold text-zinc-900 dark:text-zinc-100">{(metrics.precision * 100).toFixed(1)}%</p>
        </div>
        <div className="rounded-lg border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-900">
          <p className="text-xs text-zinc-500">Recall</p>
          <p className="text-xl font-bold text-zinc-900 dark:text-zinc-100">{(metrics.recall * 100).toFixed(1)}%</p>
        </div>
        <div className="rounded-lg border border-zinc-200 bg-white p-3 dark:border-zinc-700 dark:bg-zinc-900">
          <p className="text-xs text-zinc-500">F1 Score</p>
          <p className="text-xl font-bold text-zinc-900 dark:text-zinc-100">{(metrics.f1 * 100).toFixed(1)}%</p>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr>
              <th className="p-2 text-left text-xs text-zinc-500">Query / Doc</th>
              {DOCUMENTS.map((doc) => (
                <th
                  key={doc.id}
          ...
          <tbody>
            {QUERIES.map((query) => (
              ...
              >
                <td
                  className="cursor-pointer p-2 text-xs font-medium text-zinc-700 dark:text-zinc-300"
                    ...
                    <td key={doc.id} className="p-1">
                      <div
                        className={`flex h-10 w-10 items-center justify-center rounded text-[10px] font-medium transition-all ${
                          shouldShow ? getScoreColor(data.score, data.relevant) : "bg-zinc-100 dark:bg-zinc-800"
                        } ${aboveThreshold ? "ring-2 ring-zinc-900 dark:ring-white" : ""}`}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 text-xs">
        <div className="flex items-center gap-1">
          ...
          <span className="text-zinc-600 dark:text-zinc-400">Relevant (high score)</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="h-3 w-3 rounded bg-green-300" />
          <span className="text-zinc-600 dark:text-zinc-400">Relevant (low score)</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="h-3 w-3 rounded bg-red-400" />
          <span className="text-zinc-600 dark:text-zinc-400">Not relevant (high score) ⚠️</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="h-3 w-3 rounded bg-zinc-200 ring-2 ring-zinc-900" />
          <span className="text-zinc-600 dark:text-zinc-400">Above threshold</span>
        </div>
      </div>
